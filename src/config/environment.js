...
class EnvironmentConfig {
    ...
    loadEnvironmentFile() {
        ...
        this.config = {
            ...
            security: {
                ...
                corsOrigin: this._getCorsOrigin()
            },
    ...
    generateDevSecret() {
        if (this.isDevelopment()) {
            const devSecret = 'dev_secret_' + crypto.randomBytes(32).toString('hex');
    ...
    /**
     * Get CORS origin with production safety
     * Throws error if not configured in production
     */
    _getCorsOrigin() {
        const corsOrigin = process.env.CORS_ORIGIN;

        if (this.isProduction() && !corsOrigin) {
            throw new Error('CORS_ORIGIN must be explicitly configured in production. Using "*" is not allowed.');
        }
