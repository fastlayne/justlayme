...
console.log('[BRIDGE] JWT_SECRET configured:', `${JWT_SECRET.substring(0, 10)}...`);
...
function extractUserIdFromToken(req) {
  ...
  console.log('[BRIDGE] Auth method:', cookieToken ? 'cookie' : (headerToken ? 'header' : 'none'));

  if (token) {
    try {
      console.log('[BRIDGE] Attempting JWT verification with token:', token.substring(0, 20) + '...');
      const decoded = jwt.verify(token, JWT_SECRET);
      console.log('[BRIDGE] JWT decoded successfully, user ID:', decoded.id);
  ...
  console.log('[BRIDGE] No valid authentication found');
  return null;
}
