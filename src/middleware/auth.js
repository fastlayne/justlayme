 ...
 * UPDATED: Now uses httpOnly cookies exclusively for security and consistency
 * Reads JWT token from httpOnly cookie instead of Authorization header
 * This prevents XSS attacks and eliminates dual-auth session conflicts
 */
const authenticateToken = async (req, res, next) => {
    // Extract token from httpOnly cookie OR query parameter (for SSE)
    // Query parameter fallback is needed for EventSource which cannot send cookies or headers
    let token = req.cookies?.authToken;
    let authMethod = 'cookie';

    // SSE SUPPORT: Allow token in query parameter for EventSource connections
    if (!token && req.query.token) {
        token = req.query.token;
        authMethod = 'query';
        console.log('AUTH Using query parameter token for SSE connection');
    }
    ...
    if (!token) {
        console.log('ERROR No token provided in cookie');
 ...
 * UPDATED: Now reads from httpOnly cookie
 */
...
/**
 * Logout middleware - blacklists the token and clears cookie
 * UPDATED: Now reads from httpOnly cookie
 */
