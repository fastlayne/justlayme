...
import AnalysisProgressModal from '@/components/blackmirror/AnalysisProgressModal'
  ...
  const handleAnalysisStart = async () => {
    ...
    try {
      ...
      if (!fileContent || fileContent.trim().length === 0) {
        throw new Error('The file appears to be empty. Please upload a file with conversation data.')
      }
      console.log('✅ File read successfully, content length:', fileContent.length)

      // Estimate message count for progress display (rough estimate: ~200 chars per message)
      const estimatedMessages = Math.floor(fileContent.length / 200)
      setTotalMessages(estimatedMessages)

      // Run the ML analysis pipeline with personalization data via API
      console.log('🔄 Running ML analysis pipeline via API...')
      const personalization = getPersonalizationData()
      console.log('👤 Personalization:', personalization)

      let report

      // SMART ROUTING: Use async upload for large files (>5MB or estimated >10K messages)
      const ASYNC_THRESHOLD_SIZE = 5 * 1024 * 1024 // 5MB
      const ASYNC_THRESHOLD_MESSAGES = 10000
      const useAsyncUpload = file.size > ASYNC_THRESHOLD_SIZE || estimatedMessages > ASYNC_THRESHOLD_MESSAGES

      if (useAsyncUpload) {
        console.log(`📤 File is large (${(file.size / 1024 / 1024).toFixed(1)}MB, ~${estimatedMessages.toLocaleString()} messages)`)
        console.log('🚀 Using async job queue to avoid timeout...')

        try {
          // Upload file and create job
          const jobResponse = await relationshipAPI.uploadAsync(file, personalization)
          console.log(`✅ Job created: ${jobResponse.jobId}`)

          // Poll for job completion
          console.log('⏳ Polling for job completion...')
          report = await relationshipAPI.pollJobStatus(
            jobResponse.jobId,
            (progressData) => {
              // Update progress in UI (could enhance AnalysisProgressModal to show this)
              console.log(`📊 Progress: ${progressData.progress}% - ${progressData.message}`)
              if (progressData.progress) {
                setTotalMessages(progressData.progress) // Reuse for progress percentage
              }
            },
          ...
          if (report) {
            report = { success: true, ...report }
          }

        } catch (asyncError) {
          throw new Error(`Async analysis failed: ${asyncError.message}`)
        }
      } else {
        // Small file: use synchronous upload (old method)
        console.log(`📝 File is small (${(file.size / 1024 / 1024).toFixed(1)}MB), using synchronous analysis...`)
        try {
          report = await relationshipAPI.uploadAndAnalyze(fileContent, 'paste', personalization)
        } catch (analysisError) {
          throw new Error(`Analysis pipeline failed: ${analysisError.message}`)
        }
      }
      ...
      if (!report) {
        throw new Error('Analysis returned no results. Please try again.')
      }
      ...
      if (report.success) {
        ...
        const errorMsg = report.error || 'Analysis failed - unknown error'
            ...
            {files.length > 0 && (
              <div className="file-list">
                ...
                <button
                  ...
                  style={{marginTop: '20px', padding: '12px 32px', fontSize: '14px', touchAction: 'manipulation'}}
                >
