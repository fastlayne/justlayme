...
export async function runCompleteAnalysis(data, format = 'paste', personalization = null) {
  try {
    ...
    const allMessages = parseConversationData(data, format)

    console.log('[MLOrchestrator] Parsed messages count:', allMessages?.length || 0)
    ...
    // ARCHITECTURAL FIX: Use stratified sampling for large datasets
    // This prevents stack overflow while maintaining statistical accuracy
    const MAX_SAMPLE_SIZE = 25000 // Optimal for accuracy vs performance
    const messages = allMessages.length > MAX_SAMPLE_SIZE
      ? stratifiedSample(allMessages, MAX_SAMPLE_SIZE)
      : allMessages

    if (allMessages.length > MAX_SAMPLE_SIZE) {
      console.log(`[MLOrchestrator] Using stratified sample: ${messages.length} of ${allMessages.length} messages`)
    }

    // Step 2: Get basic stats (use full dataset for accurate counts)
    const stats = getConversationStats(allMessages)
...
/**
 * ARCHITECTURAL FIX: Stratified sampling for large datasets
 * Ensures representative sampling across:
 * - Time periods (beginning, middle, end of conversation)
 * - Both parties (sent/received balance)
 * - Message lengths (short/medium/long)
 * This maintains statistical accuracy while preventing stack overflow
 *
 * @param {Array} messages - Full message array
 * @param {number} targetSize - Target sample size
 * @returns {Array} Stratified sample
 */
function stratifiedSample(messages, targetSize) {
  if (messages.length <= targetSize) return messages

  // Split by direction to maintain sent/received balance
  const sent = messages.filter(m => m.direction === 'sent')
  const received = messages.filter(m => m.direction === 'received')

  // Calculate proportional sample sizes
  const sentRatio = sent.length / messages.length
  const sentSampleSize = Math.floor(targetSize * sentRatio)
  const receivedSampleSize = targetSize - sentSampleSize

  // Sample from each stratum using systematic sampling (preserves time distribution)
  const sampleStratum = (arr, size) => {
  ...
  const sentSample = sampleStratum(sent, sentSampleSize)
  const receivedSample = sampleStratum(received, receivedSampleSize)

  // Merge and sort by timestamp to preserve conversation flow
  const combined = [...sentSample, ...receivedSample]
  combined.sort((a, b) => {
  ...
  // Re-assign IDs to maintain continuity
  return combined.map((m, i) => ({ ...m, id: i }))
}
