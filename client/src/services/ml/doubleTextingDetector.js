...
export function detectDoubleTexting(messages) {
  ...
  const yourLongestStreak = yourStreaks.length > 0 ? Math.max(...yourStreaks.map((s) => s.length)) : 0
  ...
  const theirLongestStreak = theirStreaks.length > 0 ? Math.max(...theirStreaks.map((s) => s.length)) : 0
