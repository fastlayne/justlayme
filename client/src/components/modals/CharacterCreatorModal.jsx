...
export default function CharacterCreatorModal({ modalId, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // NEW: Check if this is a trial character
      if (newCharacter.isTrialCharacter || newCharacter.trialLimited) {
        notification.warning('Trial character - create an account to save permanently!')

        // Show signup prompt after 2 seconds
        setTimeout(() => {
          const shouldSignup = window.confirm(
            `Your character "${formData.name}" was created!\n\n` +
            'To save it permanently and create more characters, create an account now.\n\n' +
            'Without an account, it will be deleted when you refresh the page.'
          )
          if (shouldSignup) {
      ...
      // NEW: Handle trial limit error specifically
      if (error.response?.data?.code === 'TRIAL_LIMIT_REACHED') {
        notification.error('Free trial allows only 1 character. Create an account for unlimited characters!')
      } else {
